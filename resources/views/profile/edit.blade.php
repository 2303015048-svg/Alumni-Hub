        ...
        <div id="editProfileForm" class="hidden animate-fade-in">
            <div class="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm mb-6 flex justify-between items-center">
                <div>
                    ...
                    <p class="text-slate-500 text-xs mt-0.5">Silakan sesuaikan informasi profil, kontak, dan data {{ $user->role === 'kaprodi' ? 'jabatan' : 'akademik' }} Anda.</p>
                </div>
                ...
                <div class="bg-white p-8 rounded-3xl border border-slate-100 shadow-sm space-y-5">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="text" name="name" value="{{ old('name', $user->name) }}" class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm focus:bg-white {{ $user->role === 'kaprodi' ? 'focus:border-indigo-500' : 'focus:border-blue-500' }} outline-none transition" required>
                            @error('name') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                        </div>

                        <div>
                            <label class="block text-xs font-bold text-slate-700 uppercase tracking-wide mb-2">Alamat Email Sistem</label>
                            <input type="email" name="email" value="{{ old('email', $user->email) }}" class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm focus:bg-white {{ $user->role === 'kaprodi' ? 'focus:border-indigo-500' : 'focus:border-blue-500' }} outline-none transition" required>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="text" name="phone" value="{{ old('phone', $user->phone) }}" placeholder="Contoh: 08123456789" class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm focus:bg-white {{ $user->role === 'kaprodi' ? 'focus:border-indigo-500' : 'focus:border-blue-500' }} outline-none transition">
                            @error('phone') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                        </div>

                        <div>
                            <label class="block text-xs font-bold text-slate-700 uppercase tracking-wide mb-2">Alamat Domisili / Lokasi</label>
                            <input type="text" name="location" value="{{ old('location', $user->location) }}" placeholder="Contoh: Jakarta, Indonesia" class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm focus:bg-white {{ $user->role === 'kaprodi' ? 'focus:border-indigo-500' : 'focus:border-blue-500' }} outline-none transition">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div>
                            ...
                            <input type="date" name="birth_date" value="{{ old('birth_date', $user->birth_date ? \Carbon\Carbon::parse($user->birth_date)->format('Y-m-d') : '') }}" class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm focus:bg-white {{ $user->role === 'kaprodi' ? 'focus:border-indigo-500' : 'focus:border-blue-500' }} outline-none transition">
                            @error('birth_date') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                        </div>

                        <div>
                            <label class="block text-xs font-bold text-slate-700 uppercase tracking-wide mb-2">Jenis Kelamin</label>
                            <select name="gender" class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm focus:bg-white {{ $user->role === 'kaprodi' ? 'focus:border-indigo-500' : 'focus:border-blue-500' }} outline-none transition">
                        ...
                        <div>
                            ...
                            <input type="text" name="skills" value="{{ old('skills', $user->skills) }}" placeholder="Contoh: PHP, Laravel, UI/UX" class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm focus:bg-white {{ $user->role === 'kaprodi' ? 'focus:border-indigo-500' : 'focus:border-blue-500' }} outline-none transition">
                            @error('skills') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                        </div>
                    </div>

                    {{-- BLADE CONDITIONAL: Membedakan Form Input Informasi Akademik --}}
                    @if ($user->role === 'kaprodi')
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 pt-2 border-t border-slate-50">
                            <div>
                                <label class="block text-xs font-bold text-slate-700 uppercase tracking-wide mb-2">Kaprodi Program Studi</label>
                                <input type="text" name="major" value="{{ old('major', $user->major) }}" placeholder="Contoh: Informatika" class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm focus:bg-white focus:border-indigo-500 outline-none transition">
                                @error('major') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                            </div>

                            <div>
                                <label class="block text-xs font-bold text-slate-700 uppercase tracking-wide mb-2">Fakultas / Departemen</label>
                                <input type="text" name="faculty" value="{{ old('faculty', $user->faculty) }}" placeholder="Contoh: Ilmu Komputer" class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm focus:bg-white focus:border-indigo-500 outline-none transition">
                    ...
                    <div>
                        ...
                        <textarea name="about_me" rows="4" placeholder="Tuliskan deskripsi singkat diri Anda..." class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm focus:bg-white {{ $user->role === 'kaprodi' ? 'focus:border-indigo-500' : 'focus:border-blue-500' }} outline-none transition">{{ old('about_me', $user->about_me) }}</textarea>
                    ...
                    <div class="pt-2 border-t border-slate-100">
                        ...
                        <input type="password" name="current_password" class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm focus:bg-white {{ $user->role === 'kaprodi' ? 'focus:border-indigo-500' : 'focus:border-blue-500' }} outline-none transition">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="password" name="password" class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm focus:bg-white {{ $user->role === 'kaprodi' ? 'focus:border-indigo-500' : 'focus:border-blue-500' }} outline-none transition">
                        ...
                        <div>
                            ...
                            <input type="password" name="password_confirmation" class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm focus:bg-white {{ $user->role === 'kaprodi' ? 'focus:border-indigo-500' : 'focus:border-blue-500' }} outline-none transition">
                        </div>
                    </div>
                </div>

                <div class="flex justify-end">
                    <button type="submit" class="text-white px-6 py-3 rounded-xl font-bold text-sm shadow-lg transition {{ $user->role === 'kaprodi' ? 'bg-indigo-600 hover:bg-indigo-700 shadow-indigo-600/10' : 'bg-blue-600 hover:bg-blue-700 shadow-blue-600/10' }}">
        ...
        <div id="profileViewPanel" class="space-y-6">
            ...
            <div class="flex flex-col gap-1">
                <div class="flex items-center gap-2 text-xs font-medium text-slate-400">
                    ...
                    <span class="font-semibold {{ $user->role === 'kaprodi' ? 'text-indigo-600' : 'text-blue-600' }}">Profil Saya</span>
                </div>
            ...
            <div class="bg-white rounded-3xl border border-slate-100 shadow-sm p-8 flex flex-col md:flex-row items-center justify-between gap-6 relative overflow-hidden">
                <div class="flex flex-col md:flex-row items-center gap-6 z-10">
                    <div class="relative group">
                        <div class="w-28 h-28 rounded-full bg-gradient-to-tr flex items-center justify-center font-black text-white text-4xl shadow-xl border-4 border-white {{ $user->role === 'kaprodi' ? 'from-indigo-600 to-purple-600 shadow-indigo-600/10' : 'from-blue-600 to-indigo-600 shadow-blue-600/10' }}">
                    ...
                    <div class="text-center md:text-left space-y-1.5">
                        <div class="flex flex-col md:flex-row items-center gap-2">
                            ...
                            <i class="fa-solid fa-circle-check text-sm {{ $user->role === 'kaprodi' ? 'text-indigo-500' : 'text-blue-500' }}" title="Verified Account"></i>
                        </div>
                        
                        {{-- BLADE CONDITIONAL: Badge Role Utama --}}
                        @if ($user->role === 'kaprodi')
                            <span class="inline-block text-xs font-semibold text-indigo-600 bg-indigo-50 px-3 py-1 rounded-full capitalize">
                                Kaprodi {{ $user->major ?? '' }}
                            </span>
                        @else
                ...
                <button onclick="toggleEditForm()" class="text-white px-5 py-2.5 rounded-xl font-bold text-xs shadow-md transition flex items-center gap-2 self-center md:self-start z-10 {{ $user->role === 'kaprodi' ? 'bg-indigo-600 hover:bg-indigo-700 shadow-indigo-600/10' : 'bg-blue-600 hover:bg-blue-700 shadow-blue-600/10' }}">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div class="bg-white border border-slate-100 rounded-2xl p-6 shadow-sm flex flex-col justify-between">
                    <div>
                        <div class="flex items-center gap-2 text-slate-800 font-bold text-sm mb-4">
                            <i class="fa-regular fa-user text-base {{ $user->role === 'kaprodi' ? 'text-indigo-500' : 'text-blue-500' }}"></i> Tentang Saya
                        </div>
                    ...
                    <div class="mt-6 pt-4 border-t border-slate-50 space-y-2.5 text-xs font-medium">
                        ...
                        <div class="flex justify-between">
                            <span class="text-slate-400">{{ $user->role === 'kaprodi' ? 'Program Studi' : 'Pendidikan' }}</span>
                            <span class="{{ !$user->major ? 'text-slate-300 italic' : 'text-slate-700 font-semibold' }}">
                                {{ $user->major ? ($user->role === 'kaprodi' ? $user->major : 'S1 - ' . $user->major) : 'Belum diisi' }}
                            </span>
                        </div>
                    </div>
                </div>

                {{-- INFORMASI AKADEMIK / JABATAN KAPRODI --}}
                <div class="bg-white border border-slate-100 rounded-2xl p-6 shadow-sm flex flex-col justify-between">
                    <div>
                        <div class="flex items-center gap-2 text-slate-800 font-bold text-sm mb-4">
                            <i class="fa-solid text-base {{ $user->role === 'kaprodi' ? 'fa-user-tie text-indigo-500' : 'fa-graduation-cap text-blue-500' }}"></i> 
                            {{ $user->role === 'kaprodi' ? 'Informasi Jabatan' : 'Informasi Akademik' }}
                        </div>
                        <div class="space-y-3 text-xs font-medium">
                            ...
                            <div class="flex justify-between pb-2 border-b border-slate-50">
                                ...
                                <span class="font-bold {{ $user->role === 'kaprodi' ? 'text-indigo-600' : 'text-emerald-600' }}">
                                    {{ $user->role === 'kaprodi' ? 'Dosen / Kaprodi Aktif' : 'Aktif / Terverifikasi' }}
                                </span>
                            </div>
                            
                            {{-- Hanya tampil jika dia bukan Kaprodi (Mahasiswa) --}}
                            @if ($user->role !== 'kaprodi')
                                <div class="flex justify-between pb-2 border-b border-slate-50">
                    ...
                    <div class="mt-4 pt-2">
                        <span class="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-2">Bidang Keahlian</span>
                        <div class="flex flex-wrap gap-1.5">
                            @if($user->skills)
                                ...
                                @endforeach
                            @else
                                <span class="text-xs text-slate-300 italic font-medium">Belum ada keahlian dimasukkan</span>
                            @endif
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white border border-slate-100 rounded-2xl p-6 shadow-sm space-y-4">
                    <div class="flex items-center gap-2 text-slate-800 font-bold text-sm">
                        <i class="fa-solid fa-phone text-base {{ $user->role === 'kaprodi' ? 'text-indigo-500' : 'text-blue-500' }}"></i> Kontak & Alamat
                    </div>
